...
import sys
sys.path.append('.')
sys.path.append('/home/e2e/navsim_workspace')
...
def train_val(model, train_dataloader, val_dataloader, agent, cfg, device, early_stopping):
    ...
    # TensorBoard writer
    #writer = SummaryWriter(f"runs/{config.model_type}_{datetime.now().strftime('%m-%d %H:%M')}")
    ...
    for epoch in range(cfg.trainer.params.max_epochs):
        ...
        with tqdm(train_dataloader, unit = "batch") as tepoch:
            tepoch.set_description(f"Epoch {epoch+1}/{cfg.trainer.params.max_epochs}")
