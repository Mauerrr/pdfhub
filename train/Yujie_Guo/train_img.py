    ...
    for epoch in range(args.num_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if config.preprocess_choice == 'subtract_adjacent_row':
                subtract_adjacent = target_labels[:, 1:, :] - target_labels[:, :-1, :]
                decoder_input = torch.cat((ego_state_now.unsqueeze(1), target_labels[:, 0, :].unsqueeze(1), subtract_adjacent[:, :-1, :]), dim=1) # [batch_size, n_output_frames, 2] add the ego state to the first frame
            else: 
                decoder_input = target_labels[:, :-1, :] # [batch_size, n_output_frames-1, 2] delete the last frame
                decoder_input = torch.cat((ego_state_now.unsqueeze(1), decoder_input), dim=1) # [batch_size, n_output_frames, 2] add the ego state to the first frame
            
            #prediction = model(past_images, input_ego_state, decoder_input)
            #prediction = model(past_images, input_ego_state)
            prediction = greedy_decode(model, past_images, input_ego_state)
            #prediction = model.lstm_decode()
            ...
            if iteration % args.eval_freq == (args.eval_freq - 1):
                ...
                for batch_val in val_dataloader:
                    ...
                    loss_val += criterion(prediction, target_labels_processed).item()
...
def greedy_decode(model, src, input_ego_state):
    """
    Greedy decoding
    Args:
    ...
    for i in range(config.n_output_frames):
        tgt = model(src, input_ego_state, decoder_input)
        tgt_last = tgt[:, -1, :]
