...
from data_prep.Dengyuan_Hua.dataset import NuplanDataset
import data_prep.Dengyuan_Hua.config as config
from network.Dengyuan_Hua.model import UrbanDriverModel
...
def train_val(model, train_dataloader, val_dataloader, device, args, early_stopping):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            prediction = model(input_data,target_labels)#model(input_data)
            # 3 Calculate loss
            ...
            if iteration % args.eval_freq == (args.eval_freq - 1):
                ...
                if loss_val / len(val_dataloader) < best_loss:
                    best_model_path = os.path.join(endtoenddriving_Path, 'train/Dengyuan_Hua/best_model')
