...
from network.Yujie.yj_agent_torch_module import AgentPytorchModel
...
CONFIG_PATH = "."
CONFIG_NAME = "yj_default_training"
...
def train_val(model, train_dataloader, val_dataloader, agent, cfg, device, early_stopping, config):
    """
    ...
    # TensorBoard writer
    writer = SummaryWriter(f"runs/{datetime.now().strftime('%m_%d_%H_%M')}")
    ...
    for epoch in range(cfg.trainer.params.max_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            #     delta_sum = torch.zeros_like(targets['trajectory'])        
            #     # input based on equation 5
            #     current_feature = torch.zeros(features['ego_status'].shape[0], 1, features['ego_status'].shape[2])
            #     features_processed = {'ego_status': torch.cat((features['ego_status'][:, :-1, :] - features['ego_status'][:, 1:, :], current_feature), dim=1)}
            # else:
            ...
            #prediction = greedy_decode(model, features, max_len=targets.size(1), device=device)
            #prediction = model(features, targets)

            if config.preprocess_choice == 'subtract_adjacent_row':
                delta_sum = torch.zeros_like(targets['trajectory'])
                delta_sum[:, 1:, :] = torch.cumsum(prediction['trajectory'][:, :-1, :], dim=1)
                # process targets
                targets_processed = {'trajectory': targets['trajectory'] - delta_sum}
            else:
                targets_processed = targets

            # 3 Calculate loss
            loss = agent.compute_loss(features, targets_processed, prediction)
            # 4 Compute gradients
            loss.backward()
            ...
            if iteration % config.eval_freq == (config.eval_freq - 1):
                ...
                if loss_val / len(val_dataloader) < best_loss:
                    ...
                    torch.save(model.state_dict(), os.path.join(best_model_path, f'model_best_{datetime.now().strftime("%m_%d_%H_%M")}.ckpt'))
...
def main(cfg: DictConfig) -> None:
    ...
    train_val(model, train_dataloader, val_dataloader, agent, cfg, device, early_stopper, config)
