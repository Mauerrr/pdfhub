...
from network.demo.mlp_network import MLPModel
...
def train_val(cfg):
    ...
    #model = TFMModel(seq_len_in=3, seq_len_out=8, feat_size_in = 2, feat_size_out = 2, feat_size_hidden = 64, device=device).to(device)
    model = MLPModel(pretrained=True, input_n=cfg.input_n, out_features=16).to(device)
    ...
    for epoch_i in range(0, cfg.epochs):
        ...
        for train_i, (coor_past, coor_future, driving_command) in enumerate(train_dataloader):
            coor_future = coor_future[:,2:].to(device)
            ...
            #output = model(image)
            output = model(coor_past, driving_command)
        ...
        with torch.no_grad():
            ...
            for val_i, (coor_past, coor_future, driving_command) in enumerate(val_dataloader):
                coor_future = coor_future[:,2:].to(device)
                coor_past = coor_past[:,:-2].to(device)  # coor_past[:,0:-2]是后一个点相对于前一个点的增量, coor_past[:,-2:]是当前点的绝对坐标，
...
def parse_cfg():
    ...
    parser.add_argument('--save-path', type=str, default='weights/mlp_openscene', help='path to save checkpoint')
...
# python train/demo/mlp_train.py --image-root /mnt/ssd/nuscenes/trainval/ --save-path weights/mlp_nuscenes --device 1 --batch-size 64 --num-workers 8 --res-chain 0