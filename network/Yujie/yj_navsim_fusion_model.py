...
class FusionModel:
    ...
    class AllImagesEgoFusion(nn.Module):
        def __init__(self, input_feature_dim=config.n_feature_dim, output_traj_dim=3, num_encoder_layers=1,
            ...
            self.conv = torch.nn.Conv2d(in_channels=config.channel_lidar_feature, out_channels=3, kernel_size=1)
            ...
            encoder_layers = TransformerEncoderLayer(self.dim_model * 3, num_heads, dropout=dropout, batch_first=True)
            self.encoder = TransformerEncoder(encoder_layers, num_encoder_layers)
            self.out = nn.Linear(hidden_dim, output_traj_dim)
            self.final_gru = FusionModel.GRU(self.dim_model * 3).to(config.device)
        ...
        def forward(self, ego_state, images, lidar_bev):
            ...
            # print("lidar_bev: ", lidar_bev.size()) #[B, 4, 2, 256, 256]
            batch_size, preprocessed_lidar_bev = FusionModel.UtilitiesFunc.ResNetBackbone.preprocess_lidar_seq(lidar_bev)
            # print("preprocessed_lidar_bev: ", preprocessed_lidar_bev.size()) #[B*4, 2, 256, 256]
            lidar_feature = self.resnet(self.conv(preprocessed_lidar_bev))
            # print("lidar_feature: ", lidar_feature.size()) #[4B, 512]
            lidar_feature = FusionModel.UtilitiesFunc.ResNetBackbone.recover_output_features(batch_size, lidar_feature) * math.sqrt(self.dim_model)
            lidar_feature_pe = self.positional_encoder(lidar_feature)
            
            fused_features = torch.cat((ego_state_features_pe, lidar_feature_pe, images_features_pe), dim=2)
