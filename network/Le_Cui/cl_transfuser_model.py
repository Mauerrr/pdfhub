...
class Transformer(nn.Module):
    ...
    def forward(self, image_tensor, lidar_tensor):
        ...
        image_tensor_out = (
            x[:, : self.seq_len * config.img_vert_anchors * config.img_horz_anchors, :]
            .reshape(bz * self.seq_len, img_h, img_w, -1)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
        lidar_tensor_out = (
...
class TransfuserModel(nn.Module):
    ...
    def forward(self, ego_states, images, lidar):
        ...
        fused_features = self.backbone(camera_features, lidar_features)
        fused_features = fused_features.view(batch_size, self.seq_len, -1) # [B, seq_len, 512]
